...
tcp_pcb_alloc(void)
{
	...
	for (pcb = pcbs; pcb < (pcbs + TCP_PCB_SIZE); pcb++) {
...
tcp_pcb_release(struct tcp_pcb *pcb)
{
	...
	log_debug("released, local=%s, foreign=%s",
...
{
	return (int)((uintptr_t)pcb - (uintptr_t)pcbs) / sizeof(struct tcp_pcb);
}
...
tcp_output_segment(uint32_t seq, uint32_t ack, uint8_t flg, uint16_t wnd, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
	...
	pseudo.zero = 0;
	pseudo.protocol = IP_PROT_TYPE_TCP;
	total = sizeof(*hdr) + len;
	pseudo.len = hton16(total);
	psum = ~cksum16((uint8_t *)&pseudo, sizeof(pseudo), 0);
	hdr->sum = hton16(cksum16((uint8_t *)hdr, total, psum));
	...
	if (ip_output(IP_PROT_TYPE_TCP, (uint8_t *)hdr, total, local->addr, foreign->addr) == -1) {
		return -1;
	}
	return len;
}
...
tcp_retransmit_queue_cleanup(struct tcp_pcb *pcb)
{
	...
	while (1) {
		...
		log_debug("remove, seq=%u, flags=%s, len=%u", entry->seq, tcp_flg_ntoa(entry->flg), entry->len);
		free(entry);
	}
	return;
}
...
tcp_set_timewait_timer(struct tcp_pcb *pcb)
{
    ...
    log_debug("start time_wait timer: %d seconds", TCP_TIMEWAIT_SEC);
}
		...
		case TCP_PCB_STATE_LISTEN:
			...
			if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
				...
				if (pcb->mode == TCP_PCB_MODE_SOCKET) {
					...
					if (!new_pcb) {
						log_error("tcp_pcb_alloc() failure");
						return;
					}
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
	...
	if (src == IP_ADDR_BROADCAST || src == iface->broadcast || dst == IP_ADDR_BROADCAST || dst == iface->broadcast) {
		log_error("only supports unicast, src=%s, dst=%s",
...
tcp_init(void)
{
	...
	if (ip_prot_register(IP_PROT_TYPE_TCP, tcp_input) == -1) {
...
tcp_open_rfc793(struct ip_endpoint *local, struct ip_endpoint *foreign, int active)
{
	...
	if (active) {
		log_debug("active open: local=%s, foreign=%s, connecting...",
		...
		log_debug("passive open: local=%s, waiting for connection...", ip_endpoint_ntop(local, ep1, sizeof(ep1)));
		pcb->local = *local;
		if (foreign) {
			pcb->foreign = *foreign;
		}
...
AGAIN:
	...
	if (pcb->state != TCP_PCB_STATE_ESTABLISHED) {
		...
		log_error("open error: %d", pcb->state);
		pcb->state = TCP_PCB_STATE_CLOSED;
		tcp_pcb_release(pcb);
		pthread_mutex_unlock(&mutex);
		return -1;
	...
	id = tcp_pcb_id(pcb);
	log_debug("connection established: local=%s, foreign=%s",
...
tcp_connect(int id, struct ip_endpoint *foreign)
{
    ...
    pcb = tcp_pcb_get(id);
    if (!pcb) {
        log_error("pcb not found");
        pthread_mutex_unlock(&mutex);
        return -1;
    }
    if (pcb->mode != TCP_PCB_MODE_SOCKET) {
        log_error("not opened in socket mode");
        pthread_mutex_unlock(&mutex);
        return -1;
    }
    ...
    if (local.addr == IP_ADDR_ANY) {
        ...
        if (!iface) {
            log_error("ip_route_get_iface() failure");
            pthread_mutex_unlock(&mutex);
            return -1;
        }
        log_debug("select source address: %s", ip_addr_ntop(iface->unicast, addr, sizeof(addr)));
    ...
    if (!local.port) {
        for (p = TCP_SOURCE_PORT_MIN; p <= TCP_SOURCE_PORT_MAX; p++) {
            ...
            if (!tcp_pcb_select(&local, foreign)) {
                log_debug("dinamic assign srouce port: %d", ntoh16(local.port));
                pcb->local.port = local.port;
                break;
            }
        }
        if (!local.port) {
            log_debug("failed to dinamic assign srouce port");
            pthread_mutex_unlock(&mutex);
            return -1;
        }
    }
...
tcp_bind(int id, struct ip_endpoint *local)
{
    ...
    if (exist) {
        log_error("already bound, exist=%s", ip_endpoint_ntop(&exist->local, ep, sizeof(ep)));
        pthread_mutex_unlock(&mutex);
        return -1;
    }
    pcb->local = *local;
    log_debug("success: local=%s", ip_endpoint_ntop(&pcb->local, ep, sizeof(ep)));
    pthread_mutex_unlock(&mutex);
    return 0;
}
...
tcp_close(int id)
{
	...
	switch (pcb->state) {
		...
		case TCP_PCB_STATE_CLOSE_WAIT:
			...
			break;
		default:
			log_error("unknown state '%u'", pcb->state);
			pthread_mutex_unlock(&mutex);
			return -1;
	}
	if (pcb->state == TCP_PCB_STATE_CLOSED) {
...
RETRY:
	switch (pcb->state) {
		...
		case TCP_PCB_STATE_CLOSE_WAIT:
			...
			if (!iface) {
				log_error("iface not found");
				pthread_mutex_unlock(&mutex);
				return -1;
			}
			mss = ((struct net_iface *)iface)->dev->mtu - (IP_HDR_SIZE_MIN + sizeof(struct tcp_hdr));
