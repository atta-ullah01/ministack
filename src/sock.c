...
#define SOCKS_SIZE 128
static struct sock socks[SOCKS_SIZE];
...
sock_alloc(void)
{
	...
	for (entry = socks; entry < (socks + SOCKS_SIZE); entry++) {
		if (!entry->used) {
			entry->used = 1;
			return entry;
		}
...
sock_open(int domain, int type, int protocol)
{
	...
	return ((uintptr_t)s - (uintptr_t)socks) / sizeof(*s);
}
...
sock_accept(int id, struct sockaddr *addr, int *addrlen)
{
	...
	switch (s->family) {
		case AF_INET:
			...
			new_s->type = s->type;
			new_s->desc = ret;
			return (((uintptr_t)new_s - (uintptr_t)socks) / sizeof(*new_s));
	}
	return -1;
}
